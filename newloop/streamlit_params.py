...
PARAMETER_CONTROLS: tuple[ParamControl, ...] = (
    ParamControl(
        ...
        help_text="`OldLoop` forces trust, income support, mortgage assistance, VAT/prebate, and GOV surplus rebate off. `OldToNew` runs visible OldLoop quarters first, then either launches the configured `NewLoop` transition regime or turns on automation under `OldLoop` rules only.",
    ),
    ...
    ParamControl(
        ...
        240,
        1,
        help_text="Visible quarters to run in `OldLoop` before switching to `NewLoop`. In `OldToNew`, the automation start quarter is counted from this handoff.",
    ),
    ParamControl(
        ("old_to_new_launch_newloop_policies",),
        "OldToNew Launch NewLoop Policies",
        STARTUP_SECTION,
        "bool",
        help_text="When enabled, `OldToNew` switches into the configured `NewLoop` policy stack at the handoff. When disabled, the model keeps `OldLoop` policy behavior and only turns on automation so you can observe unmanaged OldLoop decay.",
    ),
    ParamControl(

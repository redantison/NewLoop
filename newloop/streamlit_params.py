...
PARAMETER_CONTROLS: tuple[ParamControl, ...] = (
    ParamControl(
        ...
        POLICY_SWITCHES_SECTION,
        "select",
        options=("NewLoop", "OldLoop", "OldToNew"),
        help_text="`OldLoop` forces trust, income support, mortgage assistance, VAT/prebate, and GOV surplus rebate off. `OldToNew` runs visible OldLoop quarters first, then follows the selected transition experiment.",
    ),
    ...
    ParamControl(
        ...
        8,
        1,
        help_text="Run this many hidden neutral startup quarters before visible Q0, with income support, trust, mortgage relief, and GOV rebate temporarily disabled. Ignored in `OldToNew`, where visible OldLoop quarters serve as the NewLoop warm-up.",
    ),
    ParamControl(
        ("old_to_new_transition_quarters",),
        "OldToNew OldLoop Quarters",
        STARTUP_SECTION,
        "int",
        0,
        240,
        1,
        help_text="Visible quarters to run in `OldLoop` before the selected OldToNew transition experiment begins. In transition modes that start automation, the automation start quarter is counted from this handoff.",
    ),
    ParamControl(
        ("old_to_new_transition_mode",),
        "OldToNew Transition Mode",
        STARTUP_SECTION,
        "select",
        options=("StayOldLoop", "AutomationOnly", "NewLoopPolicies"),
        help_text="What happens after the visible Old Loop settling period: continue OldLoop unchanged, start automation under OldLoop rules, or start automation with the NewLoop policy stack.",
    ),
    ParamControl(
        ("old_loop_wage_floor_share",),
        "Old Loop Wage Floor Share",
        STARTUP_SECTION,
        "float",
        0.0,
        1.0,
    ...
    ParamControl(
        ("old_loop_owner_housing_payment_enabled",),
        "Old Loop Owner Housing Payment",
        EXPERIMENTS_SECTION,
        "bool",
        help_text="Old Loop-only simple housing expense for owner households without active mortgage payments. This is a carrying-cost flow, not a loan.",
    ),
    ParamControl(
        ("old_loop_owner_housing_carry_rate_annual",),
        "Old Loop Owner Carry Rate / Year",
        EXPERIMENTS_SECTION,
        "float",
        0.0,
        0.20,
        0.005,
        help_text="Annual carrying-cost rate applied to owner housing value in Old Loop mode, converted to a quarterly payment.",
    ),
    ParamControl(
        ("old_loop_owner_housing_payment_income_cap",),
        "Old Loop Owner Payment Income Cap",
        EXPERIMENTS_SECTION,
        "float",
        0.0,
        0.60,
        0.01,
        help_text="Maximum owner housing payment as a share of current household cash income in Old Loop mode.",
    ),
    ParamControl(
    ...
    ParamControl(
        ("old_loop_startup_mortgage_money_seed_enabled",),
        "Old Loop Mortgage Money Seed",
        STARTUP_SECTION,
        "bool",
        advanced=True,
        help_text="Create the estimated historical mortgage-money deposits at Old Loop startup, using the configured distribution shares.",
    ),
    ParamControl(
        ("old_loop_startup_mortgage_money_seed_horizon_q",),
        "Old Loop Mortgage Seed Horizon",
        STARTUP_SECTION,
        "int",
        0,
        120,
        1,
        advanced=True,
        help_text="Visible startup quarter through which to estimate inherited mortgage principal runoff for a future mortgage-money seed. `20` includes Q0 through Q20.",
    ),
    ParamControl(
        ("old_loop_startup_mortgage_money_seed_fraction",),
        "Old Loop Mortgage Seed Fraction",
        STARTUP_SECTION,
        "float",
        0.0,
        1.0,
        0.01,
        advanced=True,
        help_text="Fraction of scheduled inherited-mortgage principal runoff used for the startup mortgage-money seed amount.",
    ),
    ParamControl(
        ("old_loop_startup_mortgage_money_seed_universal_share",),
        "Mortgage Seed Universal Share",
        STARTUP_SECTION,
        "float",
        0.0,
        1.0,
        0.05,
        advanced=True,
        help_text="Share of startup mortgage-money deposits distributed equally across all households.",
    ),
    ParamControl(
        ("old_loop_startup_mortgage_money_seed_liquidity_share",),
        "Mortgage Seed Liquidity Share",
        STARTUP_SECTION,
        "float",
        0.0,
        1.0,
        0.05,
        advanced=True,
        help_text="Share of startup mortgage-money deposits distributed by household liquidity buffer gaps and MPC.",
    ),
    ParamControl(
        ("old_loop_startup_mortgage_money_seed_seller_share",),
        "Mortgage Seed Seller Share",
        STARTUP_SECTION,
        "float",
        0.0,
        1.0,
        0.05,
        advanced=True,
        help_text="Share of startup mortgage-money deposits distributed to non-mortgagors by housing-owner/seller-history weights.",
    ),
    ParamControl(
    ...
    ParamControl(("housing_value_price_deflator_enabled",), "Deflate Housing Values With Price Level", MORTGAGES_SECTION, "bool", help_text="Experimental: scales household housing values by current price level / initial price level before mortgage turnover."),
    ParamControl(("old_loop_reissue_existing_mortgage_balance_on_turnover",), "OldLoop Reissue Existing Balance On Turnover", MORTGAGES_SECTION, "bool", help_text="Legacy shortcut: when enabled, OldLoop mortgagor turnover reissues at least the old outstanding balance rather than waiting for payoff/new issuance."),
    ParamControl(("old_loop_auto_reissue_paid_off_mortgages",), "Auto Reissue Paid-Off Mortgages", MORTGAGES_SECTION, "bool", help_text="When a household pays off a mortgage, automatically issue a fresh mortgage against the current deflated housing value."),
    ParamControl(("old_loop_paid_off_reissue_ltv",), "Paid-Off Reissue LTV", MORTGAGES_SECTION, "float", 0.0, 1.25, 0.01, help_text="Share of current deflated housing value issued as principal when a mortgage is paid off and auto-reissued."),
    ...
    ParamControl(("automation_start_quarter",), "Automation Start Quarter", "Automation", "int", 0, 240, 1, help_text="Automation remains at zero until this quarter, then the configured curve starts from its own quarter 0. In `OldToNew`, this delay is counted from the OldLoop-to-NewLoop handoff. Set above the run horizon to keep automation off for the whole run."),
    ...
    ParamControl(("price_level_initial",), "Initial Price Level", "Price & Capital", "float", 0.1, 10.0, 0.1),
    ...
    ParamControl(("old_loop_profit_markup_sensitivity",), "Old Loop Profit Markup Sensitivity", "Price & Capital", "float", 0.0, 5.0, 0.05, help_text="In Old Loop mode, add price markup pressure when lagged sector operating margins fall below their configured floors."),
    ParamControl(("old_loop_profit_markup_max",), "Old Loop Profit Markup Max", "Price & Capital", "float", 0.0, 1.0, 0.01, help_text="Maximum additional markup from the Old Loop margin-defense price rule."),
    ParamControl(("old_loop_wage_share_info",), "Old Loop IS Wage Share", STARTUP_SECTION, "float", 0.0, 1.0, 0.01, advanced=True, help_text="Old Loop-only wage share of Information-sector revenue."),
    ParamControl(("old_loop_wage_share_phys",), "Old Loop PS Wage Share", STARTUP_SECTION, "float", 0.0, 1.0, 0.01, advanced=True, help_text="Old Loop-only wage share of Physical-sector revenue."),
    ParamControl(("old_loop_margin_floor_info",), "Old Loop Margin Floor: Info", "Price & Capital", "float", -0.5, 1.0, 0.01, help_text="Lagged Info-sector operating-margin floor that triggers the Old Loop margin-defense markup."),
    ParamControl(("old_loop_margin_floor_phys",), "Old Loop Margin Floor: Physical", "Price & Capital", "float", -0.5, 1.0, 0.01, help_text="Lagged Physical-sector operating-margin floor that triggers the Old Loop margin-defense markup."),
    ...
    ParamControl(
        ...
        5.0,
        0.05,
        help_text="Experimental extra total-factor-productivity bonus for the Information sector. Capacity and productivity scale as 1 + alpha * completed automation share.",
    ),
    ParamControl(
        ("sector_tfp_alpha_phys",),
        "TFP Alpha: Physical",
        "Price & Capital",
        "float",
        0.0,
    ...
    ParamControl(
        ...
        "HH Fixed Obligation Reserve Share",
        "Price & Capital",
        "float",
        0.0,
        1.0,
    ...
    ParamControl(("hh_equity_issue_price_smoothing_q",), "HH Equity Price Smoothing (q)", "Price & Capital", "float", 0.0, 1.0, 0.01),
    ParamControl(("hh_capex_reserve_spend_rate_q",), "HH CAPEX Reserve Spend Rate (q)", "Price & Capital", "float", 0.0, 1.0, 0.01),
    ...
    ParamControl(("hh_equity_distribution_enabled",), "HH Equity Distribution Enabled", "Population", "bool", advanced=True, help_text="Allocate household private-equity dividends and wealth by a skewed household equity-owner distribution instead of broad wage weights."),
    ParamControl(("hh_equity_owner_share",), "HH Equity Owner Share", "Population", "float", 0.01, 1.0, 0.01, advanced=True, help_text="Share of households eligible to receive private-equity dividend/wealth weights, ranked by startup net worth."),
    ParamControl(("hh_equity_weight_power",), "HH Equity Weight Power", "Population", "float", 0.1, 8.0, 0.1, advanced=True, help_text="Concentration power applied to startup net worth among equity-owning households."),
    ParamControl(
...
NON_GINI_METRIC_GROUPS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ...
    (
        ...
        (
            "real_consumption",
            "real_avg_income",
            "hh_owner_housing_payment_per_h",
            "uis_per_h",
